...
const RegistrationPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        `Failed to submit registration: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    }
  };
          ...
          <div className="flex justify-between items-center mb-8 px-16">
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center">
                <div
                  className={`w-10 h-10 rounded-full ${step >= i ? "bg-[#4CAF50]" : "bg-gray-600"} flex items-center justify-center`}
                >
                  <span className="text-lg font-semibold text-white">{i}</span>
                </div>
                <div className="ml-4">
                  <p
                    className={`${step >= i ? "text-[#4CAF50]" : "text-gray-400"} font-medium`}
                  >
                    Step {i}
                  </p>
